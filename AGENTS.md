...
local session transcripts from coding agent CLIs — currently **Codex**,
...
└── agents/
    ...
    ├── Codex.rs    // ClaudeSource impl  (~/.Codex/projects/<dir>/...)
...
| ------ | ------------------------------------------------------------------- | ------------------------------- |
| Codex | `~/.Codex/projects/<dir>/<sessionId>.jsonl`                        | by project directory            |
...
- Codex: `content[].type == "image"`, `source.{base64|url}`.
...
`~/.Codex/.session-viewer-trash/` with a sibling `<file>.meta` file describing
original path, agent, project label, deletion timestamp, etc. The trash dir
lives under `~/.Codex` regardless of which agent the file came from — there
...
When a Codex `tool_result` carries a `structuredPatch`, `parse_structured_patch`
in `agents/Codex.rs` converts it into the `DiffHunk[]` shape consumed by
...
project dir, and run a per-agent CLI (`Codex --resume <id>` /
...
(`--brand` = Codex orange or Codex green) is only used for tiny accents like
